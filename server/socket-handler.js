...
module.exports = function(gameModule) {
  ...
  let User, Game, Tournament, createFriendInvite;
...
// İstatistikleri döndüren fonksiyon
  ...
  const setupSocketIO = (io) => {
    ...
    if (!User || !Game || !createFriendInvite) {
      try {
    ...
    io.on('connection', async (socket) => {
      ...
      socket.on('authenticate', async (userId) => {
        try {
          ...
          // Kullanıcı bulunamadı, hata gönder
          socket.emit('error', { 
            message: 'Kullanıcı kimliği doğrulanamadı, lütfen giriş yapın', 
            redirect: '/giris' 
          });
        } catch (error) {
      ...
      socket.on('find_match', async (timeControl) => {
        ...
        if (!socket.userId) {
          ...
          socket.emit('error', { 
            message: 'Kullanıcı kimliği bulunamadı, lütfen giriş yapın', 
            redirect: '/giris' 
          });
          return;
        }
      ...
      socket.on('join_friend_game', async ({ inviteId }) => {
        ...
        try {
          ...
          activeGames.set(gameId, {
            ...
            dbId: newGame._id,
            players: [
              { id: whitePlayerId, color: 'white', username: creatorUser.username },
              { id: blackPlayerId, color: 'black', username: joiningUser.username }
            ],
            fen: 'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1',
            pgn: '',
            moves: [],
       ...
       try {
         ...
         game.players.forEach(p => {
           ...
           if (playerSocketId) {
             ...
             const eloChange = newElo - initialElo;
